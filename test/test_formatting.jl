...
@testset "format_citation(:authoryear)" begin
    ...
    @test_diff ctext("Wilhelm2003.10132") ==
               "([Wilhelm *et al.*, 2020](@cite Wilhelm2003.10132))"
    @test_diff ctext("QCRoadmap") == "([Anonymous, 2004](@cite QCRoadmap))"
    @test_diff ctext("TedRyd") == "([Corcovilos and Weiss, undated](@cite TedRyd))"
end


@testset "strip md formatting" begin
    @test_diff _strip_md_formatting("[arXiv](https://arxiv.org)") == "arXiv"
    @test_diff _strip_md_formatting("*Title*. [arXiv](https://arxiv.org)") == "Title. arXiv"
    @test_diff _strip_md_formatting("Text with ``x^2`` math") == "Text with x^2 math"
    @test_diff _strip_md_formatting("*italics* and **bold**") == "italics and bold"
    c = IOCapture.capture() do
        paragraphs = "Multiple\n\nparagraphs\n\nare\n\nnot allowed"
...
@testset "join_bib_parts" begin
    @test_diff _join_bib_parts(["Title", "(2023)"]) == "Title (2023)."
    @test_diff _join_bib_parts(["[Title](https://www.aps.org)", "(2023)"]) ==
               "[Title](https://www.aps.org) (2023)."
    @test_diff _join_bib_parts(["Title", "arXiv"]) == "Title, arXiv."
    @test_diff _join_bib_parts(["[Title](https://www.aps.org)", "arXiv"]) ==
               "[Title](https://www.aps.org), arXiv."
    @test_diff _join_bib_parts(["Title.", "arXiv"]) == "Title. arXiv."
    @test_diff _join_bib_parts(["Title.", "[arXiv](https://arxiv.org)"]) ==
               "Title. [arXiv](https://arxiv.org)."
    @test_diff _join_bib_parts(["Title", "(2023)", "arXiv"]) == "Title (2023), arXiv."
    @test_diff _join_bib_parts(["Title", "(2023)", "Special issue."]) ==
               "Title (2023). Special issue."
    @test_diff _join_bib_parts(["Title", "``x^2``"]) == "Title, ``x^2``."
    @test_diff _join_bib_parts(["Title", "``X^2``"]) == "Title. ``X^2``."
    @test_diff _join_bib_parts(["*Title*", "``x^2``"]) == "*Title*, ``x^2``."
    @test_diff _join_bib_parts(["*Title*", "``X^2``"]) == "*Title*. ``X^2``."
    c = IOCapture.capture() do
...
@testset "format_urldate" begin
    ...
    @test_diff format_urldate(entry) == "Accessed on Oct 24, 2023"
    @test_diff format_urldate(entry; accessed_on="", fmt=dateformat"d.m.Y") == "24.10.2023"
...
@testset "format_bibliography_reference(:numeric)" begin
    ...
    @test_diff md("Luc-KoenigEPJD2004") ==
    ...
    @test_diff md("NielsenChuangCh10QEC") ==
    ...
    @test_diff md("Nolting1997Coulomb") ==
...
@testset "format_biliography_reference (preprints)" begin
    ...
    @test_diff md("NonStandardPreprint") ==
...
@testset "format_biliography_reference (alternative non-articles)" begin
    ...
    @test_diff md(numeric, "JuhlARNMRS2020X") ==
    ...
    @test_diff md(authoryear, "JuhlARNMRS2020X") ==
    ...
    @test_diff md(numeric, "Nolting1997CoulombX") ==
    ...
    @test_diff md(authoryear, "Nolting1997CoulombX") ==
    ...
    @test_diff md(numeric, "Shapiro2012X") ==
    ...
    @test_diff md(authoryear, "Shapiro2012X") ==
    ...
    @test_diff md(numeric, "PercontiSPIE2016") ==
    ...
    @test_diff md(authoryear, "PercontiSPIE2016") ==
    ...
    @test_diff md(numeric, "DevoretLH1995") ==
    ...
    @test_diff md(authoryear, "DevoretLH1995") ==
